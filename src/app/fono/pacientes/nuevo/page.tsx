...
export default function NuevoPaciente() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Validate DNI
            if (!validateDNI(formData.persona.dni)) {
                throw new Error('El DNI debe tener entre 7 y 8 dígitos');
            }

            // Validate phone number if provided
            if (formData.persona.telefono && !validatePhone(formData.persona.telefono)) {
                throw new Error('El teléfono no puede tener más de 12 dígitos');
            }

            // Validate birth date
            if (!validateBirthDate(formData.persona.fechaNac)) {
